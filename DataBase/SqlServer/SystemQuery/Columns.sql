...
li AS(
	SELECT o.schemaName,o.tableName,c.name,ISNULL(p.is_primary_key, 0) isPrimaryKey,c.is_nullable isNullable,ISNULL(ep.value, '') remark,t.name dataType,c.column_id FROM sys.columns c 
...
SELECT * FROM li t ORDER BY t.schemaName ASC,t.tableName ASC,t.column_id ASC;